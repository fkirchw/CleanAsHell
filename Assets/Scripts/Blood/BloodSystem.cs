...
public class BloodSystem : MonoBehaviour
{
    ...
    [SerializeField] private Texture2D bloodSplatTexture;
    ...
    [SerializeField] private bool enableSpreading = true;
    ...
    [SerializeField] private bool enableDripping = true;
    ...
    {
        SpawnParticles(hitPosition, strikeDirection);
    ...
    void SpawnParticles(Vector2 position, Vector2 direction)
    {
        if (bloodParticles == null) return;
        
        ParticleSystem.EmitParams emitParams = new ParticleSystem.EmitParams();
        emitParams.position = position;
        emitParams.velocity = direction * 5f;
        emitParams.startSize = Random.Range(0.1f, 0.3f);
        
        bloodParticles.Emit(emitParams, Random.Range(5, 15));
    }
    ...
    Vector2 CalculateStainCenter(Vector2 hitPosition, Vector2 direction, bool isAirborne)
    {
        if (isAirborne)
        {
            ...
            {
                return hit.point + (Vector2)direction.normalized * 0.5f;
            }
            return hitPosition + Vector2.down * 2f;
        }
        
        return hitPosition + (Vector2)direction.normalized * 0.5f;
    }
    ...
    void UpdateSpreading()
    {
        ...
        System.Array.Copy(bloodData, bloodDataBuffer, bloodData.Length);
